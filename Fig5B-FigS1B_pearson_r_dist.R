...
source("scripts_for_simulation/plot_multi_histogram.R")
...
plot_corr_coef = function(df, sliced = F){
  ...
  e$phase = 'Log'
  s$phase = 'Deceleration-Stationary'
  a$phase = 'Lag-Acceleration'
  d$phase = 'Deceleration-Stationary'
  l$phase = 'Lag-Acceleration'
  
  all = rbind(l, a, e, d, s)
  all = all[complete.cases(all$occ),]
  
  
  all$phase = factor(all$phase, levels = c("Lag-Acceleration", "Log", "Deceleration-Stationary"))
  ...
  p1 <- ggplot(all, aes(x = factor(`phase`), y = `occ`, fill = `phase`)) +
    ...
    scale_fill_manual(values = c("orange", "grey70", "royalblue"))
...
load("coef_var_r_noise_extlow.RData") #noirse from 0.001 to 0.01
