...
for (i in 1:length(subsamples)) {
  ...
  for (j in 1:length(len)) {
    ...
    if (nrow(temp)>20) {
      ...
      cors[[j]] = cor(x = temp$delta, y = temp$clr, method = "pearson")
  ...
  taxon_save[[i]] = saved
  cor_coef = data.frame(beta = unlist(betas), cor = unlist(cors))
...
summary(meta_b_all[[1]]$cor)
summary(meta_b_all[[2]]$cor)
summary(meta_b_all[[3]]$cor)
summary(meta_b_all[[4]]$cor)
...
#plot results as a boxplot -- repeat with Pearson r values
don1 = data.frame(group = "ae", value = meta_b_all[[1]]$cor)
don2 = data.frame(group = "an", value = meta_b_all[[2]]$cor)
don3 = data.frame(group = "ao", value = meta_b_all[[3]]$cor)
don4 = data.frame(group = "am", value = meta_b_all[[4]]$cor)
...
png(filename = "Pearson_r_delta_clr_4_timeseries.png", width = 4, height = 4.5, res = 300, units = "in")
print(p1)
dev.off()
