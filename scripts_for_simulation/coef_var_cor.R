coef_var_cor = function(r, k, w, plot_figs = F){
  ...
  if( plot_figs == T ){
    ...
    p0=ggplot(data=sim, aes(x=`t`))+ 
      ...
      xlab("Time")+
      labs(title = paste0("Simulated growth with stochastic x"))+
    ...
    plot_names = c("Simulated_growth_with_GR", "Simulated_growth", "Nonstochastic_growth", "Growth_rate_curve", "Acc_curve")
